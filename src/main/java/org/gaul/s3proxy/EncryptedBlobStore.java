    ...
    private PageSet<? extends StorageMetadata> filteredList(
        ...
        for (StorageMetadata sm : pageSet) {
            if (sm instanceof BlobMetadata bm) {
                ...
                // List backends that return plain StorageMetadata (no user
                // metadata or ETag): fetch the full BlobMetadata so we can
                // compute the original size before encryption padding.
                BlobMetadata bm = container == null ? null :
                    delegate().blobMetadata(container, sm.getName());
                if (bm != null) {
                    BlobMetadata fetched = bm.toBuilder()
                            .container(container)
                            .name(removeEncryptedSuffix(bm.getName()))
                            .build();
                    BlobMetadata mbm = calculateBlobSize(fetched);
                    builder.add(mbm);
            ...
            if (meta != null) {
                ...
                if (decryption.isEncrypted() &&
                    ...
                    getOptions.ranges().add(
                            String.format("%d-%d", startAt, endAt));
                }
