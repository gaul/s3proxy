...
public final class OpenStackSwiftBlobStore extends BaseBlobStore {
    ...
    public String completeMultipartUpload(MultipartUpload mpu,
        ...
        var segments = new ArrayList<SLOSegment>(sorted.size());
        for (var part : sorted) {
            segments.add(SLOSegment.create(container,
                    mpuSegmentKey(uploadId, part.partNumber()),
                    part.partETag(), part.partSize()));
        }

        String sloETag;
        try {
            sloETag = swift.objects().createStaticLargeObject(container,
                    mpu.blobName(), segments, swiftOptions);
        } catch (ResponseException re) {
